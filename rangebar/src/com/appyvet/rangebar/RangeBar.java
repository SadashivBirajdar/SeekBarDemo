...
public class RangeBar extends View {
    ...
    private static final float DEFAULT_CIRCLE_SIZE_DP = 12;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (mIsRangeBar) {
            ...
            mLeftThumb.init(ctx, yPos, expandedPinRadius, mPinColor, mTextColor, mCircleSize,
                    mCircleColor, mMinPinFont, mMaxPinFont, mArePinsTemporary, R.drawable.default_pin_image);
        }
        if (mDefaultViewId > 0) {
            mDefaultThumb = new PinView(ctx);
            mDefaultThumb.setFormatter(mFormatter);
            mDefaultThumb.init(ctx, yPos, expandedPinRadius, mPinColor, mTextColor, mCircleSize,
                    Color.parseColor("#656d78"), mMinPinFont, mMaxPinFont, mArePinsTemporary, mDefaultViewId);
        }
        ...
        mRightThumb.init(ctx, yPos, expandedPinRadius, mPinColor, mTextColor, mCircleSize,
                mCircleColor, mMinPinFont, mMaxPinFont, mArePinsTemporary, mRightViewId);
        ...
        if (mDefaultViewId > 0) {
            mDefaultThumb.setX(marginLeft + (mRightIndex / (float) (mTickCount - 1)) * barLength);
            mDefaultThumb.setXValue(getPinValue(mRightIndex));
        }
    ...
    public void setFormatter(IRangeBarFormatter formatter) {
        ...
        if (mDefaultPin && mDefaultThumb != null) {
            // mDefaultThumb.setFormatter(formatter);
        }
        if (mRightThumb != null) {
    ...
    private void rangeBarInit(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            mIsRangeBar = ta.getBoolean(R.styleable.RangeBar_rangeBar, true);
            mRightViewId = ta.getResourceId(R.styleable.RangeBar_rightViewId, R.drawable.pin_image);
            mDefaultViewId = ta.getResourceId(R.styleable.RangeBar_defaultViewId, 0);
        } finally {
            ta.recycle();
        }
    }
    ...
    private void createPins() {
        ...
        float density = getResources().getDisplayMetrics().density;
        float expandedPinRadius = mExpandedPinRadius / density;
        yPos = getHeight() - mBarPaddingBottom;
        if (mIsRangeBar) {
            mLeftThumb = new PinView(ctx);
            mLeftThumb.setFormatter(mFormatter);
            mLeftThumb.init(ctx, yPos, expandedPinRadius, mPinColor, mTextColor, mCircleSize,
        ...
        if (mDefaultViewId > 0) {
            ...
            mDefaultThumb.init(ctx, yPos, expandedPinRadius, mPinColor, mTextColor, mCircleSize,
                    ...
                    , mMinPinFont, mMaxPinFont, mArePinsTemporary, mDefaultViewId);
        }
        mRightThumb = new PinView(ctx);
        mRightThumb.setFormatter(mFormatter);
        mRightThumb.init(ctx, yPos, expandedPinRadius, mPinColor, mTextColor, mCircleSize,
        ...
        // Create the underlying bar.
        final float marginLeft = Math.max(mExpandedPinRadius, mCircleSize);
        final float barLength = getWidth() - (2 * marginLeft);
        mBar = new Bar(ctx, marginLeft, yPos, barLength, mTickCount, mTickHeightDP, mTickColor,
                mBarWeight, mBarColor);
        // Initialize thumbs to the desired indices
        if (mIsRangeBar) {
       ...
       /* if (mIsRangeBar) {
            ...
            mLeftThumb.init(ctx, yPos, 0, mPinColor, mTextColor, mCircleSize, mCircleColor,
                    mMinPinFont, mMaxPinFont, false, R.drawable.default_pin_image);
        }
        if (mDefaultPin) {
            mDefaultThumb = new PinView(ctx);
            mDefaultThumb
                    .init(ctx, yPos, 0, mPinColor, mTextColor, mCircleSize, mCircleColor, mMinPinFont,
                            mMaxPinFont, false, R.drawable.default_pin_image);
        }
        mRightThumb = new PinView(ctx);
        mRightThumb
                .init(ctx, yPos, 0, mPinColor, mTextColor, mCircleSize, mCircleColor, mMinPinFont,
                        mMaxPinFont, false, R.drawable.pin_image);
    ...
    public void setDefaultPin(boolean defaultPin) {
        mDefaultPin = defaultPin;
    }
    // Inner Classes ///////////////////////////////////////////////////////////
